...
class TestLocalJSONMemoryStore:
    ...
    async def test_save_and_retrieve(self):
        ...
        await self.store.save_incident(record)
        fetched = await self.store.get_incident(record.incident_id)
        assert fetched is not None
        assert fetched.incident_id == record.incident_id

    @pytest.mark.asyncio
    async def test_get_nonexistent_returns_none(self):
        result = await self.store.get_incident("does-not-exist")
        assert result is None

    @pytest.mark.asyncio
    async def test_find_similar_filters_by_model(self):
        r1 = _make_incident(model_urn="urn:model:a")
        r2 = _make_incident(model_urn="urn:model:b")
        await self.store.save_incident(r1)
        await self.store.save_incident(r2)
        results = await self.store.find_similar("urn:model:a")
        assert all(r.model_urn == "urn:model:a" for r in results)
        assert len(results) == 1

    @pytest.mark.asyncio
    async def test_all_incidents_ordering(self):
        for _ in range(3):
            await self.store.save_incident(_make_incident())
        all_inc = await self.store.all_incidents()
    ...
    @pytest.mark.asyncio
    async def test_claim_for_approval_works_once(self):
        record = _make_incident(with_plan=True)
        await self.store.save_incident(record)
        claimed = await self.store.claim_incident_for_approval(record.incident_id)
        assert claimed is not None
        # second claim should fail
        claimed2 = await self.store.claim_incident_for_approval(record.incident_id)
        assert claimed2 is None

    @pytest.mark.asyncio
    async def test_claim_returns_none_without_plan(self):
        record = _make_incident(with_plan=False)
        await self.store.save_incident(record)
        result = await self.store.claim_incident_for_approval(record.incident_id)
        assert result is None

    @pytest.mark.asyncio
    async def test_claim_returns_none_already_has_outcome(self):
...
class TestExecutor:
    @pytest.mark.asyncio
    async def test_no_action_executes_cleanly(self):
    ...
    @pytest.mark.asyncio
    async def test_open_ticket_without_token_returns_outcome(self):
    ...
    @pytest.mark.asyncio
    async def test_trigger_retrain_simulates(self):
    ...
    @pytest.mark.asyncio
    async def test_rollback_simulates(self):
    ...
    @pytest.mark.asyncio
    async def test_quarantine_simulates(self):
...
class TestVerifier:
    @pytest.mark.asyncio
    async def test_verifies_as_resolved_when_no_drift(self):
        ...
        result = await verifier.verify(outcome, no_drift)
        assert result.verified is True

    @pytest.mark.asyncio
    async def test_verifies_as_unresolved_when_drift_remains(self):
        plan = _make_plan(action=RemediationActionType.TRIGGER_RETRAIN)
        outcome = ActionOutcome(
            plan=plan,
            executed=True,
        ...
        result = await verifier.verify(outcome, still_drifting)
        assert result.verified is False

    @pytest.mark.asyncio
    async def test_verifier_skips_no_action(self):
        plan = _make_plan(action=RemediationActionType.NO_ACTION)
        outcome = ActionOutcome(
            plan=plan,
            executed=True,
